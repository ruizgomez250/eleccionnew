...
use App\Models\Internas\Mesa;
use App\Models\Internas\Partido;
use App\Models\Internas\Candidato;
use App\Models\Internas\VotosMesa;
use App\Models\Internas\Veedor;
use App\Models\Internas\Equipo;
use App\Models\Internas\LocalInterna;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use TCPDF;

class CertificadoInternaController extends Controller
{
    const BANCAS = [
        'intendente' => 1,
    ...
    public function __construct()
    {
        ...
        $this->middleware('can:Carga Certificados');
    ...
    public function index()
    {
        ...
        return view('certificados-internas.index', compact(
    ...
    public function getFormularioCarga(Request $request)
    {
        ...
        $html = view('certificados-internas._carga_table', compact('mesa', 'cargo', 'rows'))->render();
    ...
    {
        $request->validate([
            'mesa_id' => 'required|exists:internas_mesas,id',
            'cargo' => 'required|string',
            'preferencias' => 'required|array',
            'preferencias.*' => 'array',
            'preferencias.*.*' => 'nullable|integer|min:0',
        ]);
    ...
    {
        $request->validate(['cantidad_votos' => 'required|integer|min:0']);
        try {
            ...
            $request->validate([
                'mesa_id' => 'required|exists:internas_mesas,id',
                'partido_id' => 'required|exists:internas_partidos,id',
                'cargo' => 'required|string',
                'tipo_voto' => 'required|in:lista,preferencia',
                'cantidad_votos' => 'required|integer|min:0',
                'candidato_id' => 'nullable|exists:internas_candidatos,id',
            ]);
            ...
            return redirect()->route('certificados-internas.index')
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf->Output('resultados_internas.pdf', 'I');
    }
    ...
    public function data(Request $request)
    {
        ...
        return datatables($query)
            ...
            ->addColumn('actions', function ($v) {
                $btn = '<form action="' . route('certificados-internas.destroy', $v->id) . '" method="POST" class="form-delete d-inline">';
