...
@section('title', 'Certificados de internas')

@section('content_header')
    <div class="ua-header">
        <h1 class="ua-title"><i class="fas fa-file-signature"></i> Certificados de internas</h1>
    ...
    @if (!empty($reporteCargos))
        <div class="card card-success card-outline">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <a href="{{ route('certificados-internas.exportar.pdf') }}" class="btn btn-danger btn-sm mr-2" target="_blank">
...
<script>
    $(document).ready(function() {
        ...
        $('#distritoSelect').on('change', function() {
            ...
            if (distrito) {
                $.get('{{ route("certificados-internas.locales") }}', { distrito: distrito }, function(data) {
        ...
        $('#localSelect').on('change', function() {
            ...
            if (localId) {
                $.get('{{ route("certificados-internas.mesas") }}', { local_interna_id: localId }, function(resp) {
        ...
        $('#btnCargar').on('click', function() {
            ...
            $.get('{{ route("certificados-internas.formulario") }}', { mesa_id: mesaId, cargo: cargo }, function(resp) {
        ...
        $(document).on('blur keydown', '.voto-input', function(e) {
            ...
            $.ajax({
                url: '{{ url("certificados-internas") }}/' + id,
                type: 'PUT',
                data: {
