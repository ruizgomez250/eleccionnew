...
<div class="row mb-2">
    <div class="col-md-4">
        ...
        <x-adminlte-select2 name="equipo_punteros" id="equipo_punteros" onchange="filtrarPunterosPorEquipo()" enable-old-support>
            <option value="">Todos</option>
            @foreach ($equipos as $eq)
                <option value="{{ $eq->id }}" {{ isset($equipoSeleccionado) && $equipoSeleccionado == $eq->id ? 'selected' : '' }}>
                    {{ $eq->descripcion }}
                </option>
            @endforeach
        </x-adminlte-select2>
    ...
    <div class="col-md-4">
        ...
        <x-adminlte-select2 name="dirigente_punteros" id="dirigente_punteros" onchange="filtrarPunterosPorDirigente()" enable-old-support>
            <option value="">Todos</option>
            @foreach ($dirigentes as $dir)
                <option value="{{ $dir->id }}" {{ isset($dirigenteSeleccionado) && $dirigenteSeleccionado == $dir->id ? 'selected' : '' }}>
                    {{ $dir->nombre }}
                </option>
            @endforeach
        </x-adminlte-select2>
            ...
            <tbody>
                @foreach ($punteros as $p)
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-success btn-sm" onclick="abrirModalVotantesPuntero({{ $p->id }}, '{{ $p->nombre }}')">
                                <i class="fas fa-users"></i>
                            </button>
                            <button class="btn btn-danger btn-sm" onclick="eliminarPunteroModal({{ $p->id }}, {{ $p->id_dirigente }})">
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: xhr.responseJSON?.message || 'No se pudo eliminar el puntero'
                    });
                }
            });
        }
...
function abrirModalVotantesPuntero(punteroId, nombre) {
    // Llamar a la función que ya existe en index.blade.php
    if (typeof window.abrirModalVotantes === 'function') {
        window.abrirModalVotantes(punteroId, nombre);
    } else {
        console.error('Función abrirModalVotantes no encontrada');
    }
}
</script>